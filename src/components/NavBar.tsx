        ...
        {mobileOpen && (
          ...
          >
            <nav className="flex flex-col gap-4 text-sm text-muted-foreground">
              ...
              <NavLink href="/post-job" onClick={() => handleNav("/post-job")}>
                Post a Job
              </NavLink>
              {isAdmin && (
                <NavLink
...
function SignIn({ full = false }: { full?: boolean }) {
  const router = useRouter();
  return (
    <Button
      variant="outline"
      className={full ? "w-full" : "rounded-full px-4 py-2"}
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56">
          ...
          {isAdmin && (
            <DropdownMenuItem onClick={() => (location.href = "/admin/users")}>
              Admin Panel
            </DropdownMenuItem>
          )}
          <DropdownMenuSeparator />
